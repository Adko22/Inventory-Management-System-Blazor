...
namespace IventoryManagement.CoreBusiness.Validations
{
    public class Product_EnsurePriceIsGreaterThanInventoriesCost : ValidationAttribute
    {
        protected override ValidationResult? IsValid(object? value, ValidationContext validationContext)
        {
            ...
            if (product != null)
            {
                ...
                {
                    return new ValidationResult($"The product's price is less than the inventory cost: {TotalInventoriesCost(product).ToString("C")}!",
        ...
        private decimal TotalInventoriesCost(Product product)
        {
